...
class ImageProcessing:
    ...
    def LeftImage(self, image, event):
        ...
        elif event == 'histogram' or event == 'gaussian' or event == 'wavelet' or event == "ImgSmoothing":
            if event != "gaussian":
                image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
#https://blog.gtwang.org/programming/python-opencv-matplotlib-plot-histogram-tutorial/
        if event == "HistogramEqualization":
            ...
            if event == 'wavelet' or event == 'ImgSmoothing' or event == 'edge':
                self.Right.destroy()
            if event != 'wavelet'and event != 'ImgSmoothing' and event != 'edge':
                self.image = image
                image = cv2.cvtColor(image, cv2.COLOR_BGR2RGBA)
                image = cv2.resize(image, (512,512))
...
#https://codeinfo.space/imageprocessing/histogram-equalization/
    ...
    def ImgSmoothing(self):
        get_ans = msgbox(window, "請輸入mask大小(ex. 輸入3，會產生3x3的mask)")
        window.wait_window(get_ans.top)
...
#https://www.itread01.com/content/1542587886.html
        ans = int(get_ans.sd)
        temp = np.pad(image, int(ans/2), mode = 'constant')
        height, width = temp.shape

        for i in range(0, height-ans+1):
            for j in range(0, width-ans+1):
                image[i][j] = int((np.sum(temp[i:i+ans, j:j+ans]))/(ans*ans))
      
        self.LeftImage(self.image, "ImgSmoothing")
        self.RightImage(image, "ImgSmoothing")
...
#https://zh.wikipedia.org/wiki/%E7%B4%A2%E8%B2%9D%E7%88%BE%E7%AE%97%E5%AD%90
        #可組合出兩個濾波器
        sobel1 = np.array([[-1, 0, 1]], np.float32)
        sobel2 = np.array([[1, 2, 1]], np.float32)

        Gx1 = np.zeros((self.Right2.shape[0],self.Right2.shape[1]-2))
        Gx = np.zeros((Gx1.shape[0]-2,Gx1.shape[1]))
        Gy1 = np.zeros((self.Right2.shape[0],self.Right2.shape[1]-2))
        Gy = np.zeros((Gy1.shape[0]-2,Gx1.shape[1]))

        for x in range(0, (self.Right2.shape[0])):
            for y in range(0, (self.Right2.shape[1]-2)):
                Gx1[x,y] = np.multiply(self.Right2[x, y: y + 3], sobel1).sum()/8

        for x in range(0, (Gx1.shape[0]-2)):
            for y in range(0, (Gx1.shape[1])):
                Gx[x,y] = np.multiply(Gx1[x:x+3,y], sobel2).sum()/8

        for x in range(0, (self.Right2.shape[0])):
            for y in range(0, (self.Right2.shape[1]-2)):
                Gy1[x,y] = np.multiply(self.Right2[x, y: y + 3], sobel1).sum()/8
            
        for x in range(0, (Gy1.shape[0]-2)):
            for y in range(0, (Gy1.shape[1])):
                Gy[x,y] = np.multiply(Gy1[x:x+3,y], sobel2).sum()/8

        theta = np.arctan2(Gx, Gy)
        sobel = np.sqrt(np.add(np.square(Gx), np.square(Gy)))
        ...
        for i in range(1, h-1):
            for j in range(1, w-1):
                try:
                    ...
                    if (sobel[i,j] >= t1) and (sobel[i,j] >= t2):
                        A[i,j] = sobel[i,j]
                    else:
                        A[i,j] = 0
        ...
        #connect weak edge
        high_boundary = A.max() * 0.09
        low_boundary = high_boundary * 0.05
        H, W = A.shape
        check_boundary = np.zeros((H,W), dtype=np.int32)
        weak = np.int32(25)
        strong = np.int32(255)

        strong_i, strong_j = np.where(A >= high_boundary)
        zeros_i, zeros_j = np.where(A < low_boundary)
        weak_i, weak_j = np.where((A <= high_boundary) & (A >= low_boundary))
        check_boundary[strong_i, strong_j] = strong
        check_boundary[weak_i, weak_j] = weak
        check_boundary[zeros_i, zeros_j] = 0

        self.RightImage(check_boundary, "edge")
...
def main():
    ...
    mybutton6 = tk.Button(button_frame, text='平滑化', bg='red',command=imageProcessing.ImgSmoothing)
    mybutton6.pack(side=LEFT)
    mybutton7 = tk.Button(button_frame, text='邊緣偵測', bg='red',command=imageProcessing.edge)
